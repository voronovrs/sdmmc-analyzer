...
include_paths = ["../SaleaeAnalyzerSdk-1.1.32/include"]
link_paths = ["../SaleaeAnalyzerSdk-1.1.32/lib"]
# refers to libAnalyzer.dylib or libAnalyzer.so
link_dependencies = ["-lAnalyzer64"]

debug_compile_flags = "-O0 -w -c -fpic -g3"
release_compile_flags = "-O3 -w -c -fpic"

# loop through all the cpp files, build up the gcc command line, and attempt to
...
for cpp_file in cpp_files:
    ...
    # the output file
    release_command += " -o\"release/" + cpp_file.replace(".cpp", ".o") + "\" "
    # the cpp file to compile
    release_command += "\"" + "src/" + cpp_file + "\""

    debug_command = command
    debug_command += debug_compile_flags
    # the output file
    debug_command += " -o\"debug/" + cpp_file.replace(".cpp", ".o") + "\" "
    # the cpp file to compile
    debug_command += "\"" + "src/" + cpp_file + "\""
